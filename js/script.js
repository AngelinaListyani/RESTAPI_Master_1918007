const ApiKey = " 29e7153d97274e2ab992f0625ee30f42";
        ...
        .then(resJson => {
            ...
            resJson.teams.forEach(team => {
                ...
                <li class="collection-item avatar">
                    ...
                    <a href="#Detailbola" data-id="${team.id}" class="secondary-content"><i class="material-icons" data-id="${team.id}">info</i></a>
                </li>
                `
            });
            ...
            detail.forEach(btn => {
                btn.onclick = (event) => {
                    Detailbola(event.target.dataset.id)
                }
            })
        }).catch(err => {
            console.error(err);
...
function Detailbola(id) {
    title.innerHTML = "DETAIL BOLA";
    let bola = baseUrl + "teams/" + id

    fetch(bola, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            ...
            let regu = resJson.name;
            let pemain = resJson.shortName;
            let lokasi = resJson.address;
            let negara = resJson.phone;
            let Email = resJson.email;
            let berdiri = resJson.founded;
            let laga = resJson.venue;
            ...
            contents.innerHTML = `
                <div class="card">
                    <div class="row" style="padding-top:10px;">
                        ...
                        <div class="col s9">
                            <h6><b>${regu}</b></h6>
                            <span>Pemain  : ${pemain}</span><br>
                            <span>Maps     : ${lokasi}</span><br>
                            <span>Negara      : ${negara}</span><br>
                            <span>Email        : ${Email}</span><br>
                            <span>Berdiri      : Sejak Tahun ${berdiri}</span><br>
                            <span>Stadion  : ${laga}</span><br>
                        </div>
                    </div>
                </div>
                <h5>DAFTAR PEMAIN</h5>
